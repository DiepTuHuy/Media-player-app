...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QSlider, QHBoxLayout, QSizePolicy, QSpacerItem, QMenu, QLabel, QStackedWidget
)
...
class MinimalistVideoPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.video_widget.setMinimumSize(1, 1) # <--- THÊM DÒNG NÀY VÀO
        self.stacked_widget.addWidget(self.video_widget)

        # 3. LỚP ẢNH (Index 1)
        self.image_label = QLabel()
        self.image_label.setScaledContents(True) 
        self.image_label.mousePressEvent = self.handle_video_click 
        self.image_label.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        self.image_label.setMinimumSize(1, 1) # <--- THÊM DÒNG NÀY VÀO
    ...
    def load_media(self, filename): # Đổi tên từ load_video thành load_media
        ...
        if ext in image_exts:
            ...
            self.media_player.stop() # Dừng video nếu đang chạy
            self.stacked_widget.setCurrentIndex(1) # Chuyển sang lớp Ảnh
            
            pixmap = QPixmap(filename)
            self.image_label.setPixmap(pixmap)
            
            # Đổi kích thước cửa sổ cho vừa với ảnh
            if not pixmap.isNull():
                self.video_ratio = pixmap.width() / pixmap.height()
                self.resize(pixmap.width(), pixmap.height())
                
            self.controls_overlay.hide() # Ẩn thanh thời gian/play đi
        else:
            # ---> BẬT CHẾ ĐỘ XEM VIDEO <---
            self.is_image_mode = False
            self.stacked_widget.setCurrentIndex(0) # Chuyển sang lớp Video
            self.is_video_loaded = False 
    ...
    def media_status_changed(self, status):
        # Chỉ lấy size mặc định lần đầu tiên load video
        if status == QMediaPlayer.MediaStatus.LoadedMedia and not self.is_video_loaded:
    ...
    def zoom(self, scale_factor):
        """ Hàm xử lý phóng to/thu nhỏ giữ nguyên tỷ lệ """
        new_width = int(self.width() * scale_factor)
        # Giới hạn kích thước tối thiểu 200px và tối đa 3840px (màn 4K)
        if 200 < new_width < 3840: 
            new_height = int(new_width / self.video_ratio)
            self.resize(new_width, new_height)

    def wheelEvent(self, event):
        # Lăn chuột lên -> phóng to 10% (1.1) | Lăn xuống -> thu nhỏ 10% (0.9)
    ...
    def enterEvent(self, event): 
        if not self.is_image_mode: # Chỉ hiện overlay nếu đang xem video
...
if __name__ == "__main__":
    ...
    player = MinimalistVideoPlayer(file_to_open) # Chỗ này gọi init nó sẽ tự chạy load_media
    player.show()
    sys.exit(app.exec())